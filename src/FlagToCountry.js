...
class FlagToCountry extends Component{
    constructor(props){
        super(props);
        ...
        fetch('https://restcountries.eu/rest/v2/all').then(results => results.json())//returns promise but don't console.log anything here or you can't access the array inside in the next .then chain
          .then(data => {
              ...
              let fullQuestions = Array(10).fill().map((value, index) => {
                if(index != 0){
                  return {possibleAnswers: [countryData[index*10].country, countryData[index*10+1].country, countryData[index*10+2].country, countryData[index].country], rightAnswer: countryData[index].country, quest: countryData[index].flag};
                }
                else{
                  return {possibleAnswers: [countryData[index+100].country, countryData[index+101].country, countryData[index+102].country, countryData[index].country], rightAnswer: countryData[index].country, quest: countryData[index].flag};
                }
              });
